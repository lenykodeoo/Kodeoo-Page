    ...
    <>
      <style>{`
        *{box-sizing:border-box;margin:0;padding:0}
        ...
        body{background:#fff;font-family:system-ui,sans-serif;color:#0D0D12}
        ...
        .section-title{font-size:22px;font-weight:700;color:#0D0D12;margin-bottom:6px;letter-spacing:-0.02em;line-height:1.25}
        .section-sub{font-size:14px;color:#6B6B80;line-height:1.6;margin-bottom:20px}
        ...
        .bien-slide{min-width:100%;height:100%;object-fit:cover;display:flex;align-items:center;justify-content:center;font-size:40px;flex-shrink:0}
        ...
        .footer-btn-main{height:48px;background:#6347FF;color:#fff;border:none;border-radius:10px;font-family:inherit;font-size:14px;font-weight:600;cursor:pointer;display:flex;align-items:center;justify-content:center;gap:8px;text-decoration:none;transition:background 0.18s}
      ...
      <div className="hero">
        ...
        <div className="hero-card">
          ...
          {/* CTA Principal */}
          <button className="cta-main" onclick="openM('estimation')">
            📊 Faire estimer mon bien
          </button>

          {/* CTA Secondaires */}
          <div className="cta-secondary">
            <button className="cta-sec cta-sec-msg" onclick="openM('contact')">
              📲 Message
            </button>
      ...
      <div className="lm-card">
        ...
        <div className="lm-form">
          ...
          <button className="lm-btn" onclick="submitLead('lead_magnet')">
      ...
      <div className="estim-card">
        ...
        <div className="estim-form">
          ...
          <button className="estim-btn" onclick="submitLead('estimation')">
      ...
      <div className="avis-scroll">
        {/* Avis de démonstration — remplacés par Google Places API */}
        {[
              ...
              <div key={bien.id} className="bien-card">
                <div className="bien-slider" id={`slider-${idx}`}>
                  ...
                  {bien.photos && bien.photos.length > 1 && (
                    <>
                      <button className="bien-slider-prev" onclick={`slideB(${idx},-1,${bien.photos.slice(0,5).length})`}>‹</button>
                      <button className="bien-slider-next" onclick={`slideB(${idx},1,${bien.photos.slice(0,5).length})`}>›</button>
                ...
                <div className="bien-body">
                  ...
                  <button className="bien-cta" onclick="openM('contact')">
                    Voir ce bien →
                  </button>
                </div>
              </div>
      ...
      {(agent.instagram || agent.tiktok || agent.facebook || agent.linkedin || agent.youtube || agent.site_web) && (
        <div style={{padding:'0 16px 24px'}}>
          <div className="section-label" style={{marginBottom:14}}>Retrouvez-moi sur</div>
          <div className="socials">
            {agent.instagram && <a className="soc" href={`https://instagram.com/${agent.instagram}`} target="_blank" title="Instagram">📸</a>}
            {agent.tiktok && <a className="soc" href={`https://tiktok.com/@${agent.tiktok}`} target="_blank" title="TikTok">🎵</a>}
            {agent.facebook && <a className="soc" href={agent.facebook.startsWith('http') ? agent.facebook : `https://facebook.com/${agent.facebook}`} target="_blank" title="Facebook">👤</a>}
            {agent.linkedin && <a className="soc" href={agent.linkedin.startsWith('http') ? agent.linkedin : `https://linkedin.com/in/${agent.linkedin}`} target="_blank" title="LinkedIn">💼</a>}
            {agent.youtube && <a className="soc" href={`https://youtube.com/@${agent.youtube}`} target="_blank" title="YouTube">▶️</a>}
            {agent.site_web && <a className="soc" href={agent.site_web.startsWith('http') ? agent.site_web : `https://${agent.site_web}`} target="_blank" title="Site web">🌐</a>}
          </div>
        </div>
      )}
      ...
      <div className="footer-cta">
        ...
        <div className="footer-cta-sub">Contactez-moi directement, je réponds rapidement.</div>
        <div className="footer-cta-btns">
          <button className="footer-btn-main" onclick="openM('estimation')">
            📊 Faire estimer mon bien
          </button>
          <div className="footer-btn-row">
            <button className="footer-btn-sec" onclick="openM('contact')">📲 Message</button>
      ...
      <div className="overlay" id="m-contact">
        <div className="modal">
          ...
          <button className="m-close" onclick="closeM('contact')">×</button>
          ...
          <div className="field"><label>Vous êtes</label>
            <div className="radios">
              <div className="radio on" onclick="setR(this)">Acheteur</div>
              <div className="radio" onclick="setR(this)">Vendeur</div>
              <div className="radio" onclick="setR(this)">Autre</div>
            </div>
          </div>
          ...
          <button className="btn-sub" onclick="submitLead('contact')">Envoyer →</button>
        </div>
      </div>
      ...
      <div className="overlay" id="m-estimation">
        <div className="modal">
          <div className="m-handle"></div>
          <button className="m-close" onclick="closeM('estimation')">×</button>
          ...
          <button className="btn-sub" onclick="submitLead('estimation')">Recevoir mon estimation →</button>
        </div>
      </div>
      ...
      <script dangerouslySetInnerHTML={{__html: `
        ...
        document.querySelectorAll('.overlay').forEach(o=>{
          o.addEventListener('click',function(e){if(e.target===this){this.classList.remove('open');document.body.style.overflow=''}})
        })
        function setR(el){el.closest('.radios').querySelectorAll('.radio').forEach(r=>r.classList.remove('on'));el.classList.add('on')}
        ...
        function slideB(idx, dir, total) {
          ...
          const slides = document.getElementById('slides-'+idx)
          if (slides) slides.style.transform = 'translateX(-' + (sliderPos[idx] * 100) + '%)'
          const dots = document.querySelectorAll('#dots-'+idx+' .bien-dot')
          dots.forEach((d,i) => d.classList.toggle('on', i === sliderPos[idx]))
        }

        // Submit leads
        async function submitLead(type) {
          const agentId = '${agent.id}'
          let data = { agent_id: agentId, type }
          ...
          if (type === 'contact') {
            ...
            const adresse = document.getElementById('eAdresse') || document.getElementById('estimAdresse')
            const surface = document.getElementById('eSurface') || document.getElementById('estimSurface')
            const tel = document.getElementById('eTel') || document.getElementById('estimTel')
            const email = document.getElementById('eEmail') || document.getElementById('estimEmail')
            const typeB = document.getElementById('eType') || document.getElementById('estimType')
            data.nom = 'Prospect estimation'
            data.email = email?.value || ''
            data.telephone = tel?.value || ''
            data.message = 'Adresse: ' + (adresse?.value||'') + ' | Type: ' + (typeB?.value||'') + ' | Surface: ' + (surface?.value||'') + 'm²'
